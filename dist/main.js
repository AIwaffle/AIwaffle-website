...
{
	if (region.aV.am === region.ba.am)
	{
		return 'on line ' + region.aV.am;
	}
	return 'on lines ' + region.aV.am + ' through ' + region.ba.am;
}
...
function _Browser_visibilityInfo()
{
	return (typeof _VirtualDom_doc.hidden !== 'undefined')
		? { cO: 'hidden', cq: 'visibilitychange' }
		:
	(typeof _VirtualDom_doc.mozHidden !== 'undefined')
		? { cO: 'mozHidden', cq: 'mozvisibilitychange' }
		:
	(typeof _VirtualDom_doc.msHidden !== 'undefined')
		? { cO: 'msHidden', cq: 'msvisibilitychange' }
		:
	(typeof _VirtualDom_doc.webkitHidden !== 'undefined')
		? { cO: 'webkitHidden', cq: 'webkitvisibilitychange' }
		: { cO: 'hidden', cq: 'visibilitychange' };
}
...
var _Http_toTask = F3(function(router, toTask, request)
{
	return _Scheduler_binding(function(callback)
	{
		...
		try {
			xhr.open(request.c7, request.d5, true);
		} catch (e) {
			return done($elm$http$Http$BadUrl_(request.d5));
		}
...
function _Markdown_formatOptions(options)
{
	...
	{
		if (!lang && $elm$core$Maybe$isJust(options.a9))
		{
			lang = options.a9.a;
		}
...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {be: fragment, bj: host, by: path, bB: port_, bE: protocol, bF: query};
	});
...
var $author$project$Main$clearActivations = function (net) {
	return A2(
		$elm$core$List$map,
		function (layer) {
			return A2(
...
var $author$project$Main$updateNode = F5(
	function (layerIndex, index, combineNodes, currNet, nextNet) {
		...
		var nextNode = A2(
			...
			A2($author$project$Main$nth, index, nextLayer));
		var currLayer = A2(
			$elm$core$Maybe$withDefault,
			_List_Nil,
...
var $author$project$Main$updateActivations = F4(
	function (layerIndex, index, currNet, nextNet) {
		...
		return A5($author$project$Main$updateNode, layerIndex, index, combineNodes, currNet, nextNet);
	});
var $author$project$Main$clearActivationsExceptFirst = function (net) {
	return A4(
		$author$project$Main$updateActivations,
		0,
		0,
...
var $author$project$Main$Node = F5(
	function (x, y, pos, activation, weights) {
		return {o: activation, an: pos, af: weights, L: x, M: y};
	});
...
var $elm$http$Http$request = function (r) {
	return $elm$http$Http$command(
		$elm$http$Http$Request(
			{b7: false, cg: r.cg, cF: r.cF, bg: r.bg, c7: r.c7, d_: r.d_, bR: r.bR, d5: r.d5}));
};
var $elm$http$Http$get = function (r) {
	return $elm$http$Http$request(
		{cg: $elm$http$Http$emptyBody, cF: r.cF, bg: _List_Nil, c7: 'GET', d_: $elm$core$Maybe$Nothing, bR: $elm$core$Maybe$Nothing, d5: r.d5});
};
var $author$project$Main$getContent = function (contentName) {
	return $elm$http$Http$get(
		{
			cF: $elm$http$Http$expectString($author$project$Main$GotContent),
			d5: './contents/' + (contentName + '.md')
		});
};
var $author$project$Main$init = function (_v0) {
	...
	var _v1 = A5($author$project$Main$generateRandomNet, layers_, height_, width_, initialSeed_, $author$project$Main$generateAllLayerValues);
	var nextNet_ = _v1.a;
	var losses_ = _v1.b;
	var net_ = $author$project$Main$clearActivationsExceptFirst(nextNet_);
	return _Utils_Tuple2(
		{
			aF: 'σ',
			ay: '',
...
var $author$project$Main$backwardOneStep = function (model) {
	var currentLayerIndex = model.b.a;
	var nextLayerLength = A2(
		$elm$core$Maybe$withDefault,
		0,
		A2($author$project$Main$nth, currentLayerIndex - 1, model.r));
	var currentIndex = model.b.b;
	var nextNet = A4($author$project$Main$updateWeights, currentLayerIndex, currentIndex, model.i, model.G);
	return (currentIndex <= 0) ? ((currentLayerIndex <= 1) ? _Utils_update(
		model,
		{
			q: 0,
			b: _Utils_Tuple2(0, 0),
			i: $author$project$Main$clearActivationsExceptFirst(model.G)
		}) : _Utils_update(
		model,
		{
			b: _Utils_Tuple2(currentLayerIndex - 1, nextLayerLength - 1),
			i: nextNet
		})) : _Utils_update(
		model,
		{
			b: _Utils_Tuple2(currentLayerIndex, currentIndex - 1),
			i: nextNet
		});
};
var $author$project$Main$repeat = F3(
...
var $author$project$Main$backwardOneLayer = function (model) {
	backwardOneLayer:
	while (true) {
		var currentLayerIndex = model.b.a;
		var currentIndex = model.b.b;
		if (currentIndex > 0) {
			return A3($author$project$Main$repeat, currentIndex, $author$project$Main$backwardOneStep, model);
		} else {
			if (!currentIndex) {
				if (!currentLayerIndex) {
					return model;
				} else {
					var $temp$model = $author$project$Main$backwardOneStep(model);
					model = $temp$model;
...
var $author$project$Main$forwardOneStep = function (model) {
	var numberOfLayers = $elm$core$List$length(model.r);
	var currentLayerIndex = model.b.a;
	var layerLength = function () {
		var _v0 = A2($author$project$Main$nth, currentLayerIndex, model.r);
		if (_v0.$ === 1) {
			return 0;
		} else {
			var n = _v0.a;
	...
	var currentIndex = model.b.b;
	var nextNet = (_Utils_cmp(currentIndex, layerLength - 1) > -1) ? A4($author$project$Main$updateActivations, currentLayerIndex + 1, 0, model.i, model.G) : A4($author$project$Main$updateActivations, currentLayerIndex, currentIndex + 1, model.i, model.G);
	return (_Utils_cmp(currentIndex, layerLength - 1) > -1) ? ((_Utils_cmp(currentLayerIndex, numberOfLayers - 1) > -1) ? _Utils_update(
		model,
		{
			q: 1,
			b: _Utils_Tuple2(numberOfLayers - 1, layerLength - 1),
			G: A5($author$project$Main$generateRandomNet, model.r, model.ak, model.at, 128, $author$project$Main$generateAllLayerValues).a
		}) : _Utils_update(
		model,
		{
			b: _Utils_Tuple2(currentLayerIndex + 1, 0),
			i: nextNet
		})) : _Utils_update(
		model,
		{
...
var $author$project$Main$forwardOneLayer = function (model) {
	...
	while (true) {
		...
		var currentLayerLength = A2(
			$elm$core$Maybe$withDefault,
			0,
			A2($author$project$Main$nth, currentLayerIndex, model.r));
		var currentIndex = model.b.b;
	...
	function (msg, model) {
		switch (msg.$) {
			...
			case 1:
				...
				if (!_v1) {
					return _Utils_Tuple2(
						$author$project$Main$forwardOneStep(model),
						$elm$core$Platform$Cmd$none);
				} else {
					return _Utils_Tuple2(
						$author$project$Main$backwardOneStep(model),
						$elm$core$Platform$Cmd$none);
				}
			case 2:
				var _v2 = model.q;
				if (!_v2) {
					return _Utils_Tuple2(
						$author$project$Main$forwardOneLayer(model),
						$elm$core$Platform$Cmd$none);
				} else {
					return _Utils_Tuple2(
			...
			case 4:
				if (!model.E) {
					...
					var nextContentIndex = model.E - 1;
			...
			default:
				if (_Utils_eq(
					...
					var prevContentIndex = model.E + 1;
...
var $mdgriffith$elm_ui$Element$fromRgb = function (clr) {
	return A4($mdgriffith$elm_ui$Internal$Model$Rgba, clr.dk, clr.cL, clr.cf, clr.a2);
};
...
var $author$project$Main$edges = {aJ: 0, c0: 0, dm: 0, d0: 0};
...
var $mdgriffith$elm_ui$Internal$Style$classes = {bZ: 'a', aG: 'atv', b$: 'ab', b0: 'cx', b1: 'cy', b2: 'acb', b3: 'accx', b4: 'accy', b5: 'acr', a0: 'al', a1: 'ar', b6: 'at', aH: 'ah', aI: 'av', b9: 's', cd: 'bh', ce: 'b', ch: 'w7', cj: 'bd', ck: 'bdt', au: 'bn', cl: 'bs', av: 'cpe', cr: 'cp', cs: 'cpx', ct: 'cpy', B: 'c', ax: 'ctr', az: 'cb', aA: 'ccx', D: 'ccy', ai: 'cl', aB: 'cr', cw: 'ct', cx: 'cptr', cy: 'ctxt', cI: 'fcs', bd: 'focus-within', cJ: 'fs', cM: 'g', aK: 'hbh', aL: 'hc', bh: 'he', aM: 'hf', bi: 'hfp', cP: 'hv', cR: 'ic', cT: 'fr', cV: 'iml', cW: 'imlf', cX: 'imlp', cY: 'implw', cZ: 'it', c_: 'i', bp: 'lnk', aa: 'nb', br: 'notxt', dc: 'ol', dd: 'or', U: 'oq', dh: 'oh', bw: 'pg', bx: 'p', di: 'ppe', dn: 'ui', t: 'r', dq: 'sb', dr: 'sbx', ds: 'sby', dt: 'sbt', dw: 'e', dx: 'cap', dz: 'sev', dI: 'sk', aX: 't', dM: 'tc', dN: 'w8', dO: 'w2', dP: 'w9', dQ: 'tj', aE: 'tja', dR: 'tl', dS: 'w3', dT: 'w5', dU: 'w4', dV: 'tr', dW: 'w6', dX: 'w1', dY: 'tun', bS: 'ts', W: 'clr', d3: 'u', aY: 'wc', bW: 'we', aZ: 'wf', bX: 'wfp', a_: 'wrp'};
...
var $mdgriffith$elm_ui$Internal$Model$gridClass = $mdgriffith$elm_ui$Internal$Style$classes.b9 + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.cM);
var $mdgriffith$elm_ui$Internal$Model$pageClass = $mdgriffith$elm_ui$Internal$Style$classes.b9 + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.bw);
var $mdgriffith$elm_ui$Internal$Model$paragraphClass = $mdgriffith$elm_ui$Internal$Style$classes.b9 + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.bx);
var $mdgriffith$elm_ui$Internal$Model$rowClass = $mdgriffith$elm_ui$Internal$Style$classes.b9 + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.t);
var $mdgriffith$elm_ui$Internal$Model$singleClass = $mdgriffith$elm_ui$Internal$Style$classes.b9 + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.dw);
	...
	switch (style.$) {
		...
		case 8:
			...
			return 'grid-rows-' + (A2(
				...
				$elm$core$String$join,
				'-',
				A2($elm$core$List$map, $mdgriffith$elm_ui$Internal$Model$lengthClassName, template.v)) + ('-space-x-' + ($mdgriffith$elm_ui$Internal$Model$lengthClassName(template.dA.a) + ('-space-y-' + $mdgriffith$elm_ui$Internal$Model$lengthClassName(template.dA.b)))))));
		case 9:
			var pos = style.a;
			return 'gp grid-pos-' + ($elm$core$String$fromInt(pos.t) + ('-' + ($elm$core$String$fromInt(pos.cv) + ('-' + ($elm$core$String$fromInt(pos.at) + ('-' + $elm$core$String$fromInt(pos.ak)))))));
		case 11:
	...
	[
		...
		_List_fromArray(
			[
				...
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot(
					_Utils_ap($mdgriffith$elm_ui$Internal$Style$classes.cI, $mdgriffith$elm_ui$Internal$Style$classes.W)) + ':focus',
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'opacity', '0')
					])),
				...
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot(
					_Utils_ap($mdgriffith$elm_ui$Internal$Style$classes.cI, $mdgriffith$elm_ui$Internal$Style$classes.U)) + ':focus',
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'opacity', '1')
					])),
...
var $mdgriffith$elm_ui$Internal$Style$overrides = '@media screen and (-ms-high-contrast: active), (-ms-high-contrast: none) {' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.b9) + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.t) + (' > ' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.b9) + (' { flex-basis: auto !important; } ' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.b9) + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.t) + (' > ' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.b9) + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.ax) + (' { flex-basis: auto !important; }}' + ($mdgriffith$elm_ui$Internal$Style$inputTextReset + ($mdgriffith$elm_ui$Internal$Style$sliderReset + ($mdgriffith$elm_ui$Internal$Style$trackReset + ($mdgriffith$elm_ui$Internal$Style$thumbReset + $mdgriffith$elm_ui$Internal$Style$explainer)))))))))))))));
...
var $mdgriffith$elm_ui$Internal$Style$emptyIntermediate = F2(
	function (selector, closing) {
		return {aw: closing, j: _List_Nil, J: _List_Nil, z: selector};
	});
					...
					case 2:
						...
						return _Utils_update(
							rendered,
							{
								j: A2(
									$elm$core$List$cons,
									{aw: '\n}', j: _List_Nil, J: props, z: '@supports (' + (prop + (':' + (value + (') {' + parent.z))))},
									rendered.j)
							});
					case 4:
						var selector = rule.a;
...
var $mdgriffith$elm_ui$Internal$Style$renderCompact = function (styleClasses) {
	...
	var renderClass = function (rule) {
		...
		if (!_v2.b) {
			...
			return rule.z + ('{' + (renderValues(rule.J) + (rule.aw + '}')));
		}
	};
				...
				var modernPosition = A2(
					...
					_List_fromArray(
						[
							...
							'grid-column: ' + ($elm$core$String$fromInt(position.cv) + (' / ' + ($elm$core$String$fromInt(position.cv + position.at) + ';')))
						]));
				var _class = '.grid-pos-' + ($elm$core$String$fromInt(position.t) + ('-' + ($elm$core$String$fromInt(position.cv) + ('-' + ($elm$core$String$fromInt(position.at) + ('-' + $elm$core$String$fromInt(position.ak)))))));
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					A2(
					$mdgriffith$elm_ui$Internal$Model$bracket,
					'.' + (name + ('.' + ($mdgriffith$elm_ui$Internal$Style$classes.dx + ('> .' + ($mdgriffith$elm_ui$Internal$Style$classes.aX + (', .' + (name + (' .' + ($mdgriffith$elm_ui$Internal$Style$classes.dx + (' > .' + $mdgriffith$elm_ui$Internal$Style$classes.aX)))))))))),
					_List_fromArray(
						[
...
var $mdgriffith$elm_ui$Internal$Model$adjust = F3(
	function (size, height, vertical) {
		return {ak: height / size, bN: size, bU: vertical};
	});
...
var $mdgriffith$elm_ui$Internal$Model$convertAdjustment = function (adjustment) {
	var lines = _List_fromArray(
		[adjustment.cp, adjustment.cc, adjustment.cz, adjustment.c1]);
	var lineHeight = 1.5;
					...
					return A3(
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$elm$html$Html$Attributes$class($mdgriffith$elm_ui$Internal$Style$classes.b9 + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.dw))
									]))
							]));
			}
		}();
					...
					_List_fromArray(
						[
							...
							A2(
								...
								' ',
								_List_fromArray(
									[$mdgriffith$elm_ui$Internal$Style$classes.b9, $mdgriffith$elm_ui$Internal$Style$classes.dw, $mdgriffith$elm_ui$Internal$Style$classes.ax, $mdgriffith$elm_ui$Internal$Style$classes.D, $mdgriffith$elm_ui$Internal$Style$classes.b5])))
						]),
					_List_fromArray(
					...
					_List_fromArray(
						[
							...
							A2(
								...
								' ',
								_List_fromArray(
									[$mdgriffith$elm_ui$Internal$Style$classes.b9, $mdgriffith$elm_ui$Internal$Style$classes.dw, $mdgriffith$elm_ui$Internal$Style$classes.ax, $mdgriffith$elm_ui$Internal$Style$classes.D, $mdgriffith$elm_ui$Internal$Style$classes.b3])))
						]),
					_List_fromArray(
						[html])) : html));
			case 1:
					...
					_List_fromArray(
						[
							...
							A2(
								...
								' ',
								_List_fromArray(
									[$mdgriffith$elm_ui$Internal$Style$classes.b9, $mdgriffith$elm_ui$Internal$Style$classes.dw, $mdgriffith$elm_ui$Internal$Style$classes.ax, $mdgriffith$elm_ui$Internal$Style$classes.b4])))
						]),
					_List_fromArray(
					...
					_List_fromArray(
						[
							...
							A2(
								...
								' ',
								_List_fromArray(
									[$mdgriffith$elm_ui$Internal$Style$classes.b9, $mdgriffith$elm_ui$Internal$Style$classes.dw, $mdgriffith$elm_ui$Internal$Style$classes.ax, $mdgriffith$elm_ui$Internal$Style$classes.b2])))
						]),
					_List_fromArray(
						[html])) : html));
			default:
...
var $mdgriffith$elm_ui$Internal$Model$textElementClasses = $mdgriffith$elm_ui$Internal$Style$classes.b9 + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.aX + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.aY + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.aL)))));
...
var $mdgriffith$elm_ui$Internal$Model$textElementFillClasses = $mdgriffith$elm_ui$Internal$Style$classes.b9 + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.aX + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.aZ + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.aM)))));
						...
						switch (location) {
							case 0:
								return A2(
									...
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.aa, $mdgriffith$elm_ui$Internal$Style$classes.dw, $mdgriffith$elm_ui$Internal$Style$classes.bZ]));
							case 1:
								return A2(
									$elm$core$String$join,
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.aa, $mdgriffith$elm_ui$Internal$Style$classes.dw, $mdgriffith$elm_ui$Internal$Style$classes.ce]));
							case 2:
								return A2(
									$elm$core$String$join,
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.aa, $mdgriffith$elm_ui$Internal$Style$classes.dw, $mdgriffith$elm_ui$Internal$Style$classes.dd]));
							case 3:
								return A2(
									$elm$core$String$join,
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.aa, $mdgriffith$elm_ui$Internal$Style$classes.dw, $mdgriffith$elm_ui$Internal$Style$classes.dc]));
							case 4:
								return A2(
									$elm$core$String$join,
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.aa, $mdgriffith$elm_ui$Internal$Style$classes.dw, $mdgriffith$elm_ui$Internal$Style$classes.cT]));
							default:
								return A2(
									$elm$core$String$join,
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.aa, $mdgriffith$elm_ui$Internal$Style$classes.dw, $mdgriffith$elm_ui$Internal$Style$classes.cd]));
						}
					}())
				]),
			_List_fromArray(
			...
			return (portion === 1) ? _Utils_Tuple3(
				...
				_List_fromArray(
					[
						A3(
						$mdgriffith$elm_ui$Internal$Model$Single,
						$mdgriffith$elm_ui$Internal$Style$classes.b9 + ('.' + ($mdgriffith$elm_ui$Internal$Style$classes.t + (' > ' + $mdgriffith$elm_ui$Internal$Style$dot(
...
var $author$project$Main$getCurrentNode = function (model) {
	var layerIndex = model.b.a;
	var index = model.b.b;
	return A2(
		$elm$core$Maybe$withDefault,
		$author$project$Main$emptyNode,
		A2(
			...
			A2(
				$elm$core$Maybe$withDefault,
				_List_Nil,
				A2($author$project$Main$nth, layerIndex, model.G))));
};
...
var $author$project$Main$highlightHelper = F2(
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					$author$project$Main$toElmUIColor($avh4$elm_color$Color$yellow)) : $mdgriffith$elm_ui$Element$Border$color(
					A4($mdgriffith$elm_ui$Element$rgba, 0, 0, 0, 0))
				]),
			$mdgriffith$elm_ui$Element$text(
				A2($myrho$elm_round$Round$round, 2, n)));
	});
var $author$project$Main$highlight = function (n) {
	return A2($author$project$Main$highlightHelper, n, false);
};
var $author$project$Main$highlightWithBorder = function (n) {
	return A2($author$project$Main$highlightHelper, n, true);
};
	...
	function (attrs, children) {
		...
		if (spaced.$ === 1) {
			return A4(
				...
				A2(
					$elm$core$List$cons,
					$mdgriffith$elm_ui$Internal$Model$htmlClass($mdgriffith$elm_ui$Internal$Style$classes.ai + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.D + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.a_)))),
					A2(
						$elm$core$List$cons,
						$mdgriffith$elm_ui$Element$width($mdgriffith$elm_ui$Element$shrink),
						A2(
				...
				if (!padded.$) {
					...
					return ((_Utils_cmp(r, (x / 2) | 0) > -1) && (_Utils_cmp(b, (y / 2) | 0) > -1)) ? $elm$core$Maybe$Just(
						$mdgriffith$elm_ui$Element$paddingEach(
							{aJ: b - ((y / 2) | 0), c0: l - ((x / 2) | 0), dm: r - ((x / 2) | 0), d0: t - ((y / 2) | 0)})) : $elm$core$Maybe$Nothing;
				} else {
					return $elm$core$Maybe$Nothing;
				}
			}();
...
var $author$project$Main$calculationDisplay = function (model) {
	...
	var currWeights = currNode.af;
	var currLayerIndex = model.b.a;
	var prevLayer = A2($author$project$Main$nth, currLayerIndex - 1, model.i);
	var currAcitvation = currNode.o;
	var _v0 = model.q;
	if (!_v0) {
		if (prevLayer.$ === 1) {
				...
				function () {
					if (!terms.b) {
						...
						return A2(
							...
							A2(
								$elm$core$List$cons,
								$mdgriffith$elm_ui$Element$text(model.aF + '('),
								_Utils_ap(
									A3(
										$elm$core$List$foldl,
										F2(
												...
												return _Utils_ap(
													...
													_List_fromArray(
														[
															...
															$author$project$Main$highlightWithBorder(activation),
															$mdgriffith$elm_ui$Element$text(' * '),
															$author$project$Main$highlight(weight)
														]));
											}),
										_List_Nil,
										terms),
									_List_fromArray(
										[
											$mdgriffith$elm_ui$Element$text(') = '),
											$author$project$Main$highlightWithBorder(currAcitvation)
										]))));
					}
				}());
		}
...
var $mdgriffith$elm_ui$Element$column = F2(
	function (attrs, children) {
		return A4(
			...
			A2(
				$elm$core$List$cons,
				$mdgriffith$elm_ui$Internal$Model$htmlClass($mdgriffith$elm_ui$Internal$Style$classes.cw + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.ai)),
				A2(
					$elm$core$List$cons,
					$mdgriffith$elm_ui$Element$height($mdgriffith$elm_ui$Element$shrink),
					A2(
...
var $author$project$Main$GetNextContent = {$: 5};
var $author$project$Main$GetPreviousContent = {$: 4};
...
var $author$project$Main$MoveOneLayer = {$: 2};
		...
		return A4(
			...
			A2(
				...
				A2(
					...
					A2(
						$elm$core$List$cons,
						$mdgriffith$elm_ui$Internal$Model$htmlClass($mdgriffith$elm_ui$Internal$Style$classes.aA + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.D + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.dt + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.br)))))),
						A2(
							$elm$core$List$cons,
							$mdgriffith$elm_ui$Element$pointer,
							A2(
...
var $author$project$Main$controlButton = $mdgriffith$elm_ui$Element$Input$button(
	_List_fromArray(
		[
...
var $author$project$Main$layerStepControl = $author$project$Main$controlButton(
	{
		aN: $mdgriffith$elm_ui$Element$text('Move 1 Layer'),
		bt: $elm$core$Maybe$Just($author$project$Main$MoveOneLayer)
	});
var $author$project$Main$AdjustLearningRate = function (a) {
	return {$: 0, a: a};
};
...
var $mdgriffith$elm_ui$Element$Input$slider = F2(
	function (attributes, input) {
		...
		var factor = (input.d6 - input.c8) / (input.c6 - input.c8);
		var _v0 = input.dZ;
						...
						_List_fromArray(
							[
								...
								$elm$html$Html$Attributes$max(
									$elm$core$String$fromFloat(input.c6))),
								$mdgriffith$elm_ui$Internal$Model$Attr(
								$elm$html$Html$Attributes$value(
									$elm$core$String$fromFloat(input.d6))),
	...
	return A2(
			...
			]),
		{
			aN: A2(
				...
				$mdgriffith$elm_ui$Element$text(
					'Learning Rate: ' + A2($myrho$elm_round$Round$round, 2, model.al))),
			c6: 5,
			c8: 0,
			db: $author$project$Main$AdjustLearningRate,
			dG: $elm$core$Maybe$Nothing,
			dZ: $mdgriffith$elm_ui$Element$Input$defaultThumb,
			d6: model.al
		});
};
...
var $author$project$Main$MoveOneStep = {$: 1};
var $author$project$Main$stepControl = $author$project$Main$controlButton(
	{
		aN: $mdgriffith$elm_ui$Element$text('Move 1 Step'),
		bt: $elm$core$Maybe$Just($author$project$Main$MoveOneStep)
	});
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				$author$project$Main$stepControl,
				$author$project$Main$layerStepControl
			]));
};
...
var $author$project$Main$directionTracker = function (model) {
	...
	var background = function () {
		var lastLayerIndex = $elm$core$List$length(model.r) - 1;
		var lastIndex = A2(
			$elm$core$Maybe$withDefault,
			1,
			A2($author$project$Main$nth, lastLayerIndex, model.r)) - 1;
		var _v0 = model.q;
		if (!_v0) {
			return _Utils_eq(
				model.b,
...
var $mdgriffith$elm_ui$Internal$Model$optionsToRecord = function (options) {
	...
	return andFinally(
		A3(
			$elm$core$List$foldr,
			combine,
			{cI: $elm$core$Maybe$Nothing, cP: $elm$core$Maybe$Nothing, c9: $elm$core$Maybe$Nothing},
			options));
};
			...
			A2(
				...
				$mdgriffith$elm_ui$Internal$Model$htmlClass(
					A2(
						...
						' ',
						_List_fromArray(
							[$mdgriffith$elm_ui$Internal$Style$classes.dn, $mdgriffith$elm_ui$Internal$Style$classes.b9, $mdgriffith$elm_ui$Internal$Style$classes.dw]))),
	...
	function (settings, point, str) {
		return A2(
			...
			settings,
			{
				C: _List_Nil,
				Y: $joakin$elm_canvas$Canvas$Internal$Canvas$NotSpecified,
				Z: $joakin$elm_canvas$Canvas$Internal$Canvas$DrawableText(
					{aP: $elm$core$Maybe$Nothing, bA: point, aX: str})
			});
	});
...
var $joakin$elm_canvas$Canvas$renderTextFill = F5(
	function (txt, x, y, color, cmds) {
		return A2(
			$elm$core$List$cons,
			A4($joakin$elm_canvas$Canvas$Internal$CustomElementJsonApi$fillText, txt.aX, x, y, txt.aP),
			A2(
				$elm$core$List$cons,
...
var $joakin$elm_canvas$Canvas$renderTextStroke = F5(
	function (txt, x, y, color, cmds) {
		return A2(
			$elm$core$List$cons,
			A4($joakin$elm_canvas$Canvas$Internal$CustomElementJsonApi$strokeText, txt.aX, x, y, txt.aP),
			A2(
				$elm$core$List$cons,
	...
	function (x, y, t, cmds) {
		return A2(
			...
			function () {
				if (!t.$) {
					var image = t.a;
					return A9($joakin$elm_canvas$Canvas$Internal$CustomElementJsonApi$drawImage, 0, 0, image.at, image.ak, x, y, image.at, image.ak, image.c$);
				} else {
					var sprite = t.a;
					var image = t.b;
					return A9($joakin$elm_canvas$Canvas$Internal$CustomElementJsonApi$drawImage, sprite.L, sprite.M, sprite.at, sprite.ak, x, y, sprite.at, sprite.ak, image.c$);
				}
			}(),
			cmds);
	});
		...
		return A3(
			...
			F2(
				function (width, height) {
					return {ak: height, c$: target, at: width};
				}),
			A2(
				$elm$json$Json$Decode$at,
				_List_fromArray(
	...
	function (options, attrs, entities) {
		return A3(
			...
			A2(
				...
				A2(
					$elm$core$List$cons,
					$elm$html$Html$Attributes$height(options.ak),
					A2(
						$elm$core$List$cons,
						$elm$html$Html$Attributes$width(options.at),
						attrs))),
			A2(
				$elm$core$List$cons,
...
var $author$project$Main$neuralNet = function (model) {
	var isVisitedNode = F2(
		function (node, currentPosition) {
			var nodeLayerIndex = node.an.a;
			var nodeIndex = node.an.b;
	...
	var isCurrentNode = F2(
		function (node, currentPosition) {
			return _Utils_eq(node.an, currentPosition);
		});
			...
			[
				...
				_List_fromArray(
					[
						...
						A2(isVisitedNode, node, model.b) ? $author$project$Main$greenScale(node.o) : $author$project$Main$greyScale(node.o)),
						$joakin$elm_canvas$Canvas$Settings$stroke(
						A2(isVisitedNode, node, model.b) ? $avh4$elm_color$Color$yellow : $avh4$elm_color$Color$black),
						$joakin$elm_canvas$Canvas$Settings$Line$lineWidth(model.H * 0.1)
					]),
				_List_fromArray(
					[
						A2(
				...
				_List_fromArray(
					[
						...
						{
							bc: 'sans-serif',
							bN: $elm$core$Basics$round(model.H * 0.8)
						}),
						$joakin$elm_canvas$Canvas$Settings$Text$align(2),
						$joakin$elm_canvas$Canvas$Settings$Text$baseLine(2),
						$joakin$elm_canvas$Canvas$Settings$fill(
						$author$project$Main$highContrast(node.o))
					]),
				_Utils_Tuple2(node.L, node.M),
				A2($myrho$elm_round$Round$round, 2, node.o))
			]);
	};
	var displayLosses = function () {
		var width = model.H * 2;
		var height = model.H * 2;
		return A3(
			$elm$core$List$map2,
			F2(
				function (node, loss) {
						...
						[
							...
							_List_fromArray(
								[
									...
									$joakin$elm_canvas$Canvas$Settings$Line$lineWidth(model.aj)
								]),
							_List_fromArray(
								[
									A3(
									$joakin$elm_canvas$Canvas$rect,
									_Utils_Tuple2(node.L + width, node.M - (height / 2)),
									width,
									height)
								])),
							A3(
							...
							_Utils_Tuple2(node.L + ((width * 3) / 2), node.M),
							A2($myrho$elm_round$Round$round, 2, loss))
						]);
				}),
			function () {
				var _v2 = $elm_community$list_extra$List$Extra$last(model.i);
				if (_v2.$ === 1) {
					return _List_Nil;
				} else {
					var layer = _v2.a;
	...
	var displayLayers = function (func) {
		return $author$project$Main$flatten2D(
			A3(
				...
				func,
				function () {
					var _v1 = $elm_community$list_extra$List$Extra$init(model.i);
					if (!_v1.$) {
						var initial = _v1.a;
		...
		function (start, end, weight) {
			return A2(
				...
				_List_fromArray(
					[
						$joakin$elm_canvas$Canvas$Settings$stroke(
						A2(isVisitedNode, start, model.b) ? $author$project$Main$greenScale(weight) : $author$project$Main$greyScale(weight)),
						$joakin$elm_canvas$Canvas$Settings$Line$lineWidth(
						A2(isCurrentNode, start, model.b) ? (model.aj * 2) : model.aj)
					]),
				_List_fromArray(
					[
						A2(
						$joakin$elm_canvas$Canvas$path,
						_Utils_Tuple2(start.L, start.M),
						_List_fromArray(
							[
								$joakin$elm_canvas$Canvas$lineTo(
								_Utils_Tuple2(end.L, end.M))
							]))
					]));
		});
	var displayEdges = F2(
	...
	return A3(
		...
		_Utils_Tuple2(model.at, model.ak),
		_List_Nil,
		_Utils_ap(
			clearBackground,
			_Utils_ap(
				...
				[
					...
					_List_fromArray(
						[
							...
							$author$project$Main$calculationDisplay(model))
						]),
...
var $author$project$Main$main = $elm$browser$Browser$element(
	{cU: $author$project$Main$init, dK: $author$project$Main$subscriptions, d4: $author$project$Main$update, d7: $author$project$Main$view});
